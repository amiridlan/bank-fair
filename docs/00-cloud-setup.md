...
1. **Create the repo** at github.com/new: name `fairops`. Leave **Add a README**, **.gitignore**, and **license** all unticked. The Angular scaffold must not collide with existing files.
...
4. **Install the Claude GitHub App** on the repo (required for a private repo; recommended anyway): github.com/apps/claude/installations/new → **Only select repositories** → `fairops`.
...
1. At claude.ai/code, select repo `fairops`, branch `main`.
2. Set mode:
...
| Claude reports Node.js is below 22.12 | Edit the environment (cloud icon above the message box → gear icon) and add a setup script: `npm install -g n && n 22 && hash -r && node -v`. Start a new session |
